...
Whole thing essentially written by Andy and Julia
"""
...
class Fluorophores:
    ...
    - attributes:
        - id: unique id for each molecule since molecule order isn't preserved
        - orientations: Orientation class object containing molecules' Euclidean positions and local times
        - state_info: FluorophoreStateInfo class object containing possible fluorophore states
        - states: current state of each molecule, initialized to initial_state
        - transition_times: molecules' transition times based on exponential distribution sampling
            of initial state's lifetime
        - transition_events: a record of each transition event for sake of photon emission simulation
    - methods:
        - phototransition: simulates a phototransition between two states
        - time_evolve: simulates rotational diffusion after a specified delta_t
        - get_xyz_for_state: returns Euclidean xyz of all molecules in a state
        - get_xyzt_at_transitions: returns Euclidean xyz and time points at which a specific transition occurred
        - delete_fluorophores_in_state: deletes all fluorophores in a specific state for memory purposes.
...
def _test_fluorophores_anisotropy_decay_plot(n=int(1e8)):
    ...
    # this is very much for the perfect system. but true enough, don't mistake it as principled though.
...
class FluorophoreStateInfo:
    ...
    - attributes:
        - list: list of states
        - dict: state names
        - num: the state number / index
        - orphan_state: a final state not defined in the list of states
        - valid: false if there is a state not defined in the list of states
    - methods:
        - clear: reinitializes the object to blank, same as init.
        - add: adds a fluorophore state [FluorophoreState] to list of possible states
        - validate: makes sure its final state is a possible state
        - n and lifetime: returns index and lifetime of the states.
    """
    def __init__(self):
        self.clear()

    def clear(self):
        # TODO: why this function? why not just do this in the init?
        self.list = []
        self.dict = {}
        self.validate()
    ...
    def validate(self):
        self.valid = True  # Optimistic! TODO: should this be in init?
        self.num = {}  # TODO: should this be in init?
        for n, state in enumerate(self.list):
            ...
            for final_state in state.final_states:
                if not final_state in self.dict:
                    self.orphan_state = (state.name, final_state)  # TODO: should this be in init?
                    self.valid = False
        return None
    ...
    def __getitem__(self, x):
        ...
        Returns state name if passed a string.
        Returns the state at an index if passed an integer.
        """
        if not self.valid:
            raise LookupError(
    ...
    def __contains__(self, x):
        """Checks if a state is in the state info object."""
        try:
            self[x]  # TODO does this actually do what it's supposed to?
            return True
        except (KeyError, IndexError):
            return False

    def __len__(self):
        """The length is reported as number of states in the object."""
...
class FluorophoreState:
    ...
    """
    A dataclass to store a fluorophore state and its info.
    - attributes:
        - final_states: a string or list/tuple of strings, probably should correspond to other fluorophore state names.
        - lifetime: a positive float
        - name: a string
        - probabilities: a 1D array-like of positive floats that sums to 1, corresponds to final_states entries.
    - methods:
        None
    """
    def __init__(
...
class Orientations:
    ...
    - attributes:
        - diffusion_time: Positive number(s) corresponding to each molecule's diffusion time
        - n: The integer number of molecules
        - t: The corresponding molecule's current local time, initialized to 0.
        - x, y, z: The corresponding molecule's current Euclidean position, initialized randomly.
    - methods:
        - time_evolve: Takes a diffusive rotational step corresponding to specified delta_t and updates t, z, y, x.
    """
    def __init__(
        self,
        number_of_molecules,
